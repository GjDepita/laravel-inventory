                ...
                <div class="p-6 text-gray-900 dark:text-gray-100">
                    ...
                    <div class="overflow-x-auto">
                        <div x-data="productModal()">
                            ...
                            <!-- Labeling Modal -->
                            <div x-show="showLabelingModal" x-cloak x-transition
                                class="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
                                <div class="bg-white rounded-lg shadow-lg p-6 w-11/12 md:w-1/2">
                                    <h2 class="text-xl font-semibold text-gray-800 mb-4">Enter ASIN & FNSKU for Product: <span x-text="product.title"></span> </h2>

                                    <label class="block text-gray-700">ASIN:</label>
                                    <input type="text" x-model="asin" class="w-full border text-gray-600 rounded px-3 py-2 mb-4">

                                    <label class="block text-gray-700">FNSKU:</label>
                                    <input type="text" x-model="fnsku" class="w-full border text-gray-600 rounded px-3 py-2 mb-4">

                                    <button @click="saveLabelingData()"
                                            class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded w-full">
                                        Save
                                    </button>

                                    <button @click="showLabelingModal = false"
                                            class="mt-4 bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded w-full">
                                        Cancel
                                    </button>
                                </div>
                            </div>
    ...
    <script>
        function productModal() {
            return {
                ...
                openLabelingModal(product) {
                    this.product = product;
                    // Reset ASIN and FNSKU fields
                    this.labelingProductId = product.id;
                    this.asin = '';
                    this.fnsku = '';
                    this.showLabelingModal = true;
                },

                saveLabelingData() {
                    fetch(`/products/save-labeling/${this.labelingProductId}`, {
                        method: 'POST',
                        headers: {
                        ...
                        if (response.ok) {
                            alert('Labeling data saved!');
                            location.reload();
                        } else {
                            alert('Failed to save labeling data.');
                        }
                    });
                }
            }
